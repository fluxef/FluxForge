...
impl PostgresDriver {
    ...
    pub fn map_to_postgres_write_type(&self, internal_type: &str, config: &ForgeConfig) -> String {
        ...
        if let Some(write_types) = config.get_type_list("postgres", "on_write") {
            if let Some(mapped) = write_types.get(&lower) {
                return mapped.clone();
            }
        }
        lower
    ...
    pub fn field_migration_sql(&self, field: &ForgeColumn, config: &ForgeConfig) -> String {
        ...
        let mut type_sql = if field.auto_increment {
            match t.as_str() {
                ...
                _ => format!("{} GENERATED BY DEFAULT AS IDENTITY", pg_type),
            }
        } else {
            pg_type.clone()
        };
    ...
    pub fn map_row_to_universal_values(
        ...
        for (i, col) in row.columns().iter().enumerate() {
            ...
            let val = match type_name {
                "INT2" | "SMALLINT" | "SMALLSERIAL" => ForgeUniversalValue::Integer(
                    row.try_get::<i16, _>(i).map_err(to_decode_err)? as i64,
                ),
                "INT4" | "INTEGER" | "SERIAL" => ForgeUniversalValue::Integer(
                    row.try_get::<i32, _>(i).map_err(to_decode_err)? as i64,
                ),
                "INT8" | "BIGINT" | "BIGSERIAL" => {
                    ForgeUniversalValue::Integer(row.try_get::<i64, _>(i).map_err(to_decode_err)?)
                }
                "FLOAT4" | "REAL" => ForgeUniversalValue::Float(
                    row.try_get::<f32, _>(i).map_err(to_decode_err)? as f64,
                ),
                ...
                s if s == "INT2[]" || s == "SMALLINT[]" => {
                    ...
                    let arr = v
                        .into_iter()
                        .map(|x| serde_json::Value::from(x as i64))
                        .collect();
