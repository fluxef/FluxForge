...
impl PostgresDriver {
    ...
    pub fn field_migration_sql(&self, field: &ForgeColumn, _config: &ForgeConfig) -> String {
        // Determine base type, handling auto-increment via IDENTITY to avoid explicit sequence names
        let t = field.data_type.to_lowercase();
        let mut type_sql = if field.auto_increment {
            match t.as_str() {
                ...
                _ => format!("{} GENERATED BY DEFAULT AS IDENTITY", field.data_type),
            }
        } else {
    ...
    pub fn map_row_to_universal_values(
        ...
        for (i, col) in row.columns().iter().enumerate() {
            ...
            let val = match type_name {
                ...
                "TIMESTAMPTZ" => {
                    let dt_utc = row.try_get::<chrono::DateTime<chrono::Utc>, _>(i).map_err(to_decode_err)?;
                ...
                s if s == "INT2[]" || s == "SMALLINT[]" => {
                    ...
                    let arr = v.into_iter().map(|x| serde_json::Value::from(x as i64)).collect();
                ...
                s if s == "INT8[]" || s == "BIGINT[]" => {
                    ...
                    let arr = v.into_iter().map(|x| serde_json::Value::from(x)).collect();
                    ForgeUniversalValue::Json(serde_json::Value::Array(arr))
                }
                s if s == "TEXT[]" || s == "VARCHAR[]" || s == "CHAR[]" || s == "BPCHAR[]" || s == "NAME[]" => {
                ...
                s if s == "FLOAT4[]" || s == "REAL[]" => {
                    ...
                    let arr = v.into_iter().map(|x| serde_json::Value::from(x as f64)).collect();
                    ForgeUniversalValue::Json(serde_json::Value::Array(arr))
                }
...
impl DatabaseDriver for PostgresDriver {
    ...
    async fn fetch_schema(&self, config: &ForgeConfig) -> Result<ForgeSchema, Box<dyn Error>> {
        ...
        for table in &mut tables {
            ...
            let pk_cols: Vec<String> = pk_rows.into_iter().map(|r| r.get::<String, _>("column_name")).collect();
                    ...
                    for col in &columns {
                        ...
                        match val {
                            ...
                            ForgeUniversalValue::Json(_) => {
                                value_sql_parts.push(format!("${}::jsonb", arg_index));
                                arg_index += 1;
                            }
                            _ => {
