#[cfg(test)]
mod tests {
    use fluxforge::core::{ForgeConfig, ForgeColumn, ForgeTable, ForgeIndex, ForgeSchema};
    ...
    fn test_map_postgres_type() {
        ...
        pg_read_map.insert("timestamp without time zone".to_string(), "datetime".to_string());
    ...
    fn test_alter_table_migration_sql_add_column() {
        ...
        source_table.columns.push(ForgeColumn::new("email", "varchar"));

        let mut target_table = ForgeTable::new("users");
        target_table.columns.push(ForgeColumn::new("id", "int4"));

        let stmts = driver.alter_table_migration_sql(&source_table, &target_table, &config, false).unwrap();
